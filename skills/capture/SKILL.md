...
Capture has exactly seven root nouns: `session`, `page`, `tab`, `measure`, `motion`, `cdp`, and `lib`. `page` contains `elements`, `click`, `type`, `shot`, `navigate`, `exec`, and `scroll`; `tab` contains `launch`, `quit`, `list`, `open`, `close`, `reset`, and `network`; traffic and external logs are read through `session har` and `session log`. For the compact tree, shared contracts, and a representative flow, read [cli-reference.md](cli-reference.md).
...
- Use `tab` for browser lifecycle, endpoint discovery, tab lifecycle, and connection-level network emulation. When no browser is running, `tab launch` starts one capture owns and reaps; never spawn a browser yourself, because nothing reaps that. Capture never signals a browser it did not start.
