/** Routing stub for `page scroll` — emits a structured not_implemented
...
export async function cmdPageScroll(parsed: ParsedArgs, _args: string[]): Promise<void> {
  emitResult(
    {
      tag: 'error',
      attrs: { command: 'page scroll', code: 'not_implemented' },
      summary: text`received: \`page scroll\`; this leaf is routed but not yet implemented.`,
    },
    { json: parsed.json },
  );
  process.exit(1);
