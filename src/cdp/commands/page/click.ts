/** Routing stub for `page click` — emits a structured not_implemented
...
export async function cmdPageClick(parsed: ParsedArgs, _args: string[]): Promise<void> {
  emitResult(
    {
      tag: 'error',
      attrs: { command: 'page click', code: 'not_implemented' },
      summary: text`received: \`page click\`; this leaf is routed but not yet implemented.`,
    },
    { json: parsed.json },
  );
  process.exit(1);
