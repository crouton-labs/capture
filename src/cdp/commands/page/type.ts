/** Routing stub for `page type` — emits a structured not_implemented
...
export async function cmdPageType(parsed: ParsedArgs, _args: string[]): Promise<void> {
  emitResult(
    {
      tag: 'error',
      attrs: { command: 'page type', code: 'not_implemented' },
      summary: text`received: \`page type\`; this leaf is routed but not yet implemented.`,
    },
    { json: parsed.json },
  );
  process.exit(1);
