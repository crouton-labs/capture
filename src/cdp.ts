 ...
 * and `cdpMain` (the routed command dispatch). HAR ownership is NOT exported
 * here: the session HAR manager (`src/har-manager.ts`) fed by the internal
 * streaming motion collector (`src/cdp/host/collectors/motion.ts`, which
 * directly imports `src/cdp/har-recorder.ts`) is the sole HAR lane.
