...
type LeafOptions = {
  ...
  readonly recovery?: string; readonly whenToUse?: string;
};
...
function leaf(path: string, description: string, options: LeafOptions = {}): RegistryLeaf {
  ...
  return {
    ...
    help: {
      description, whenToUse: options.whenToUse ?? `Choose this when you need ${description.toLowerCase()}`,
...
function branch(path: string, description: string, children: readonly RegistryNode[], model?: string, root = false, whenToUse?: string): RegistryBranch {
  return { kind: 'branch', path, summary: description, visibility: 'public', children, help: {
    description, whenToUse: whenToUse ?? `Choose this when working with ${description.toLowerCase()}`,
...
export const CAPTURE_REGISTRY: RegistryBranch = branch('', 'Capture — browser evidence and automation over CDP.', [
  ...
  branch('measure', 'immutable snapshots and factual rendered-structure reads.', [
    ...
    branch('measure variation', 'compare states, distributions, or controlled environments.', [
      ...
      leaf('measure variation sweep', 'acquire a controlled variation sweep.', { flags: [value('--session', 'exact-session-id'), value('--target', 'target-token'), port(), value('--navigation-timeout', '1..60000', undefined, undefined, 'ms'), requiredValue('--axis', 'viewport-width|viewport-height', ['viewport-width', 'viewport-height']), requiredValue('--from', '1..16384'), requiredValue('--to', '1..16384'), value('--samples', '2..64', undefined, 5), value('--viewport-width', '1..16384'), value('--viewport-height', '1..16384'), bool('--freeze-animations'), timeout('--settle-timeout'), bool('--capture-unsettled'), bool('--pixels')], mutualExclusions: [['--session', '--target'], ['--session', '--port']], effects: effects(true, true, true), constraints: 'Selects one existing target: --session <exact-id>, or --target <token> with optional --port, or scoped active-session omission. --port is accepted only with --target (with --session it fails port_conflicts_with_session; alone it fails sweep_port_requires_target). A positional URL is forbidden. --navigation-timeout is recognized but always fails sweep_navigation_timeout_forbidden (no fresh-url mode here). Raw selection ignores ambient CDP_PORT, CDP_TARGET, and CDP_HAR_ID; all selection failures precede probes, locks, mutation, and allocation; resolution/locking matches snap. Axis is exactly viewport-width or viewport-height. --from and --to are safe integers 1..16384 with from < to. --samples <2..64> default 5, and samples must be <= to-from+1. A width sweep requires singleton --viewport-height <1..16384>; a height sweep requires singleton --viewport-width <1..16384>. Snap acquisition flags allowed are --freeze-animations, --settle-timeout <1..60000> default 5000, --capture-unsettled, --pixels; state-mutation flags, legacy ranges/set-files/gates are rejected before probes, locks, mutation, allocation.', orderedOutput: 'Endpoint/session attestation; exact axis/range/sample values; immutable observation manifest; ascending sampled snapshot IDs/directories; transitions/uncertainties; coverage; bounded rows/omissions; restoration state.', artifactOwnership: 'Creates immutable sampled snapshots and one observation/recovery manifest, never a derived read.', recovery: 'sweep_port_requires_target: add --target <token>, or remove --port to use the scoped active session. port_conflicts_with_session: remove --port; session routes always use their recorded endpoint. sweep_navigation_timeout_forbidden: remove --navigation-timeout and retry the same existing-target sweep. Correct axis, --from, --to, --samples, or the orthogonal --viewport dimension through capture measure variation sweep -h. A failed sample points to its recovery manifest with every finalized partial snapshot. snapshot_publication_owner_live: let the reported live publication finish or terminate that exact owner before retrying; no index entry was published. snapshot_index_recovery_failed: restore filesystem access for the reported final/quarantine paths and retry; no index entry was published.' }),
    ], 'Variation children select ordered pair comparison, ordered-set distribution, or controlled target sampling; direct reads own facts within one snapshot.', false, 'Choose this when the question spans snapshots or controlled environments rather than one settled state; use the direct read children for facts within one snapshot.'),
