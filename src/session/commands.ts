...
output:
  ...
  including the ones this list bounds away. A live HAR file is line-delimited
  JSON (NDJSON): one header record followed by entry batches; read it one line
  at a time. A stopped session's bundled har.json is one ordinary JSON document.
  \`--body\` writes one complete body to
