...
export const RunRecordSchema = z.object({
  ...
  grade: z.object({ correctness: z.boolean(), evidenceComplete: z.boolean(), firstClassCapability: z.enum(["used", "absent", "unavailable"]), cdpAccounting: z.object({ unaccounted: z.number().int().nonnegative(), rule: z.string() }), callRatio: z.number().nonnegative().nullable(), elapsedRatio: z.number().nonnegative().nullable(), stdoutTokenRatio: z.number().nonnegative().nullable(), referenceDrift: z.array(z.enum(["hostClass", "chromeBuild", "captureBuildHash"])).optional(), finalClass: FinalClassSchema, reasons: z.array(z.string()) }),
