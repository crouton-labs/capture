...
export async function startFixture({ caseId, variant = "faulty", runId, port = 0, fixtureDir, oracleDir }) {
  ...
  return { url, port: address.port, requestLog, async reset() { if (typeof fixture.reset === "function") await fixture.reset(context); }, async preflight(tools) { if (typeof fixture.preflight !== "function") throw new Error(`Fixture ${caseId} does not provide preflight()`); return fixture.preflight({ ...tools, url, variant }); }, async stop() { await new Promise((resolveClose, rejectClose) => server.close((error) => error ? rejectClose(error) : resolveClose())); } };
}
