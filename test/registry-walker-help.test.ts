...
test('descriptor help renders descriptor-owned measurement selection and recovery contracts', () => {
  ...
  for (const row of [
    ...
    "Choose this for one element's cascade, ink box, overflow/clip-path clipping chain, stacking context, size, text, or form provenance; use geometry for distance or nominal intersection between two elements and map for topology across the page.",
  ...
  for (const section of [
    'capture measure explain\n\nread one element’s captured structural and ink provenance.',
    'Constraints: One explicit snapshot and required singleton --selector element reference; optional singleton --state <name> selects its recorded post-force target values for border/ink and clipping comparisons; optional bounded --size, --text, and --form sections.',
    'Output: Explicit snapshot, --selector, and requested base or forced state; attestation/identity; cascade winners; border and derived ink boxes with named contributors and base/forced-state provenance or explicit unavailability; overflow padding-box and clip-path source measurements; stacking, scroll, and requested sections.',
    'Artifact ownership: Reads immutable snapshot artifacts only and creates no artifact, derived read, crop, browser mutation, or session mutation.',
    'Recovery: If the explicit snapshot or --selector is unavailable, inspect its exact resolver reason and source availability, then retry with one available css:, backend:, axid:, ax:, or text: reference from that immutable snapshot. A requested state without a target row is reported factually and retains the base-state answer; ink evidence remains unavailable when the snapshot did not capture the target border box or ink-relevant computed styles; reacquire when that evidence is required.'
...
test('real grammar declarations include value flags, CDP method plus optional params, and exact pagination census', () => {
  ...
  assert.deepEqual(start.flags.map((flag) => [flag.name, flag.grammar, flag.required, flag.repeatable]), [['--url', 'absolute-http(s)-url', undefined, undefined], ['--target', 'target-token', undefined, undefined], ['--hold', 'boolean', undefined, undefined], ['--port', '1..65535', undefined, undefined]]);
  ...
  const snapConstraints = 'Constraints: Exactly one target mode from the four-row matrix: fresh-url = one absolute http(s) URL with optional --port; named-session = --session <exact-id> with no --port; explicit-target = --target <token> with optional --port; active-session = none of url/--session/--target. --port alone fails port_requires_url_or_target; --port with either session route fails port_conflicts_with_session; conflicting modes fail snapshot_target_conflict; more than one positional fails unexpected_positional. --navigation-timeout <1..60000> integer ms default 10000 is valid only in fresh-url mode (else navigation_timeout_requires_fresh_url). --settle-timeout <1..60000> integer ms default 5000. State and target tokens are case-sensitive. --state <name[:css-selector]> repeats; the nine names are normal, hover, focus, active, checked, open, disabled, invalid, all; all expands in that fixed order excluding normal and conflicts with every other state; the first colon splits name from a nonempty selector and later colons belong to the selector; duplicate normalized (state,selector) requests are rejected while distinct selectors for one state stay distinct. Each supported force records the target plus up to 100 target-descendant/following-peer-sibling-subtree rows; truncation is recorded in states.json. Each forced row writes a state PNG and target-border-plus-padding raster diff with CSS-pixel scale and changed-pixel bbox; --state-padding <0..16384> selects its padding (default 8). --list-crops opts into the complete per-crop artifact listing; by default output reports only the count and crops directory. --viewport <width>x<height> with each dimension a base-10 safe integer 1..16384. CDP_PORT, CDP_TARGET, CDP_HAR_ID are ignored. Every syntax/mode error fails before endpoint probes, navigation, mutation, lock acquisition, or durable allocation.';
