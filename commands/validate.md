...
Capture has exactly seven root nouns: `session`, `page`, `tab`, `measure`, `motion`, `cdp`, and `lib`. `page` contains `elements`, `click`, `type`, `shot`, `navigate`, `exec`, and `scroll`; `tab` contains `launch`, `quit`, `list`, `open`, `close`, `reset`, and `network`; traffic and external logs are read through `session har` and `session log`.

Choose the evidence grain from branch help: `page` for live-tab targeting, actions, or visual orientation; `measure` for settled static facts; `motion` for facts over time; `tab` for browser lifecycle (`tab launch` starts a browser capture owns and reaps, rather than hand-rolling one) and connection plumbing; and `session` for scoped traffic and artifacts. Use `cdp` only when the wrapped branches do not expose the needed protocol fact.
